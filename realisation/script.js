...
function handleApiError(error, context = 'Réalisation') {
                        ...
                        <div class="gallery-card" style="cursor: pointer;" onclick="openImageZoom(${index})">
                            <div class="gallery-card-image">
                                <img src="${getImageUrl(img)}" 
                                     alt="${img.caption || getImageAlt(img, realisation.title + ' - Image ' + (index + 1))}" 
                                     loading="lazy">
                            </div>
...
async function refreshRealisationInBackground(realisationId) {
    try {
        ...
        const freshData = await fetchAPI(`/realisations/${realisationId}`, 120000);
        if (freshData) {
